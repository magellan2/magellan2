 ...
 * is numerical and does not change between reports of different turns, it can therefore be used for
 * merging reports. In order to access a resource in a region this id proves unuseful, though.
...
public class RegionResource implements Unique {
  ...
  /**
   * This method allows to set the id of this resource even after object creation. It should be use
   * with care as ids are often used as map keys or similar objects and changing them will have
  ...
  public boolean equals(Object o) {
    ...
    try {
      ...
      return getType().getID().equals(((RegionResource) o).getType().getID());
  ...
  public static void merge(GameData curGD, RegionResource curRes, GameData newGD,
    ...
    /*
     * the constructor enforces a valid id and type, so we do not need to set them here
     */
    if (sameTurn) {
