...
public class TreeHelper {
  ...
  private static final Comparator<Named> shipComparator = nameComparator;
    ...
    private static final Comparator<Unit> groupCmp = new UnitGroupComparator(new NameComparator<Unique>(null), null, null);

    private static final Comparator<Unit> factionDisguisedCmp = new UnitFactionDisguisedComparator(null);
    
    private static final Comparator<Unit> combatCmp = new UnitCombatStatusComparator(null);
    private static final Comparator<Unit> factionCmp = new UnitFactionComparator<Faction>(new FactionTrustComparator<Named>(NameComparator.DEFAULT), null);
    private static final Comparator<Unit> trustlevelCmp = UnitTrustComparator.DEFAULT_COMPARATOR;
    private static final Comparator<Taggable> taggableCmp  = new TaggableComparator(TAGGABLE_STRING,  null);
    private static final Comparator<Taggable> taggableCmp2 = new TaggableComparator(TAGGABLE_STRING2, null);
    private static final Comparator<Taggable> taggableCmp3 = new TaggableComparator(TAGGABLE_STRING3, null);
    private static final Comparator<Taggable> taggableCmp4 = new TaggableComparator(TAGGABLE_STRING4, null);
    private static final Comparator<Taggable> taggableCmp5 = new TaggableComparator(TAGGABLE_STRING5, null);
    ...
    for(Iterator<Unit> iter = units.iterator(); iter.hasNext();) {
      ...
      if(sortCriteria >= treeStructure.length) {
        ...
        for(Iterator<TempUnit> tempUnits = curUnit.tempUnits().iterator(); tempUnits.hasNext();) {
          ...
          UnitNodeWrapper tempNodeWrapper = factory.createUnitNodeWrapper(tempUnit, tempUnit.getPersons());
        ...
        if(change(treeStructure[sortCriteria], curUnit, prevUnit)) { 
            ...
            case FACTION:
              ...
              retVal += addUnits(factionNode, treeStructure, sortCriteria + 1, helpList, factory, activeAlliances, unitNodes, data);
              helpList.clear();
...
//              DefaultMutableTreeNode groupNode = new DefaultMutableTreeNode(simpleGroupNodeWrapper);
              ...
              retVal += addUnits(groupNode, treeStructure, sortCriteria + 1, helpList, factory, activeAlliances, unitNodes, data);
            } else {
              retVal += addUnits(mother, treeStructure, sortCriteria + 1, helpList, factory, activeAlliances, unitNodes, data);
            }
        ...
        case COMBAT_STATUS:
            SimpleNodeWrapper combatNodeWrapper = factory.createSimpleNodeWrapper(MagellanFactory.combatStatusToString(prevUnit),
                                                "kampfstatus");
        ...
        case TRUSTLEVEL:
            SimpleNodeWrapper trustlevelNodeWrapper = factory.createSimpleNodeWrapper(FactionTrustComparator.getTrustLevelLabel(prevUnit.getFaction().getTrustLevel()),
                                                null);
    ...
    if(!helpList.isEmpty()) {
      ...
      if(sortCriteria <= treeStructure.length) {
        ...
        case COMBAT_STATUS:
          ...
          node = new DefaultMutableTreeNode(factory.createSimpleNodeWrapper(MagellanFactory.combatStatusToString(curUnit),"kampfstatus"));
        ...
        case TRUSTLEVEL:
          node = new DefaultMutableTreeNode(
                            factory.createSimpleNodeWrapper(FactionTrustComparator.getTrustLevelLabel(curUnit.getFaction().getTrustLevel()), null));
