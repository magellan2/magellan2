...
public class GroupEditorTableModel extends AbstractTableModel {
  ...
  protected Map<Group, Map<Faction,AllianceState>> newGroupStates = new HashMap<Group, Map<Faction,AllianceState>>();
  ...
  public void setOwner(Faction owner) {
    ...
    for (Faction faction : factions) {
      if (faction.getID().equals(owner.getID())) continue; // ignore yourself
      addRow(faction);
    }
  ...
  public Object getValueAt(int rowIndex, int columnIndex) {
    ...
    if (columnIndex == 0) {
      ...
      if (newStates.containsKey(faction)) return newStates.get(faction);
  ...
  public void save() {
    ...
    if (!newStates.isEmpty()) {
      ...
      for (Unit unit : owner.units()) {
        if (unit.getGroup() != null) continue;
  ...
  public Class<?> getColumnClass(int c) {
    ...
    if (o == null) return super.getColumnClass(c);
    return o.getClass();
  }
  ...
  /**
   * Returns the string representation of the allied state of a given faction
   * towards the given group.
   */
