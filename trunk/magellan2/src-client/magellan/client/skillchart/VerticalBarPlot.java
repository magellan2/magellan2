 ...
 *      because the method I needed to change (draw) accesses a private method (getBars). I added
 *      the method getBarAreas() to receive a Vector with the areas of the bars on the screen, that
 *      is needed in SkillChartJFreeChartPanel to show tooltips on them.
 */
public class VerticalBarPlot extends BarPlot implements VerticalValuePlot {
  ...
  public VerticalBarPlot(JFreeChart chart, Axis horizontal, Axis vertical, Insets insets,
      int introGap, int trailGap, int categoryGap, int seriesGap) throws AxisNotCompatibleException {
  ...
  /**
   * Sets the vertical axis for the plot. This method should throw an exception if the axis doesn't
  ...
  double calculateBarWidth(Rectangle2D plotArea) {
    ...
    double usable =
        plotArea.getWidth() - introGap - trailGap - ((categoryCount - 1) * categoryGap)
  ...
  /**
   * Checks the compatibility of a vertical axis, returning true if the axis is compatible with the
   * plot, and false otherwise.
   * 
  ...
  public void draw(Graphics2D g2, Rectangle2D drawArea) {
    if (insets != null) {
      drawArea =
          new Rectangle2D.Double(drawArea.getX() + insets.left, drawArea.getY() + insets.top,
              drawArea.getWidth() - insets.left - insets.right, drawArea.getHeight() - insets.top
                  - insets.bottom);
    }
    ...
    Rectangle2D plotArea =
        new Rectangle2D.Double(drawArea.getX() + vAxisArea.getWidth(), drawArea.getY(), drawArea
            .getWidth()
            - vAxisArea.getWidth(), drawArea.getHeight() - h);
  ...
  /**
   * Returns a list of bars that will fit inside the specified area. Note that the list of bars will
   * be empty if the data source is empty.
   */
      ...
      while (iterator.hasNext()) {
        ...
        for (int seriesIndex = 0; seriesIndex < seriesCount; seriesIndex++) {
          ...
          if (barVisible) {
            ...
            bars.add(new Bar(barX, top, barWidth, base - top, new BasicStroke(), outlinePaint,
                paint));
          }
        }
  ...
  public Number getMinimumVerticalDataValue() {
    DataSource data = getChart().getDataSource();
  ...
  /**
   * Returns the maximum value in either the domain or the range, whichever is displayed against the
   * vertical axis for the particular type of plot implementing this interface.
   */
